...
var
  ...
  ,getListOrVal = l => (l && (1 in l)) ? l : l?.[0]

export var rawPropsSymbol = Symbol('raw')

export function retrieveNodes (startPoint ,endPoint = startPoint) {
...
function buildComponent (fn ,props = {} ,children) {
  ...
  if (Array.isArray(props)) props = Object.fromEntries(length = props)
  else length = undefined

  props = { ...props ,[rawPropsSymbol]: props }

  if (length) for (idx = length.length - 1 ;idx > -1 ;idx--) {
    let v = length[idx][1]

    if (typeof v != 'function') (props[length[idx][0]] = () => v)
  }
  else {
    length = Object.keys(props)

    for (idx = length.length - 1 ;idx > -1 ;idx--) {
      let v = props[length[idx]]

      if (typeof v != 'function') (props[length[idx]] = () => v)
    }
  }

  props.children = children

  createImmediateEffect(() => (nodes = resolveNodes(untrack(fn ,props))))

  return nodes
}

function buildXML ({tagName ,attributes ,children}) {
  var node ,directive ,directives

  for (idx = 0 ,length = attributes.length ;idx < length ;idx++) {
    if (
      (typeof ((directive = attributes[idx])[0]) == 'function') ||
      (typeof ((directive = attributes[idx])[0]?.[0]) == 'function')
    ) directives ? directives.push(directive) : (directives = [directive])

    else if (
      (typeof (directive = attributes[idx][0]) == 'object') &&
      !Array.isArray(directive)
    ) attributes.push(...Object.entries(directive))

    else directive = undefined

    if (directive) (attributes.splice(idx, 1) ,idx-- ,length--)
  }
  ...
  for (idx = 0 ,length = directives?.length ;idx < length ;idx++) {
    if (typeof (directive = directives[idx][0]) == 'function') directive(node ,directives[idx][1])
    else ((directive = directive[0]) ,directive(node ,directives[idx][1] ,directives[idx][0][1]))
  }
